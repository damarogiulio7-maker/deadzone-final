...
import { createGameState, stepSim, applyAction } from '/shared/sim.js';
...
function computeLocalInput() {
  ...
  const me = gs && gs.players[myIdx];
  let angle = lastLocalInput.angle;
  if (input.touch.aim.active) angle = input.touch.aim.angle;
  else if (me) angle = Math.atan2((input.mouse.y + camY) - me.y, (input.mouse.x + camX) - me.x);
...
function frame(ts) {
  ...
  if (gameRunning && gs) {
    ...
    if (me) {
      if (isSolo) {
        me.input.dx = inp.dx; me.input.dy = inp.dy; me.input.angle = inp.angle; me.input.firing = inp.firing;
      }
    }
